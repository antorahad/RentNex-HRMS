...
const HomeContent = () => {
    ...
    const pendingBillsCount = bill.filter(item => item.billstatus === 'Pending' && item.billtype === 'Running').length;

    const receiveBillsCount = bill.filter(item => item.billstatus === 'Received' && item.billtype === 'Running').length;
    return (
                ...
                <div className="w-full lg:w-1/2 grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="card bg-orange-600 rounded-lg hover:scale-105 transition-transform duration-300 ease-in-out">
                        <div className="card-body text-white">
                            ...
                            <h2 className="text-2xl font-bold">Received Bill</h2>
                            <p className="text-xl font-bold">{receiveBillsCount} Bill</p>
                        </div>
                    </div>
